...
export default function Crypto() {
  ...
  const stablecoinUsd = positions
    .filter(p => (CRYPTO_CATEGORY[p.ticker] || "Autres") === "Stablecoin")
    .reduce((s, p) => s + Number(p.quantity) * (Number(p.market_price_usd) || 0), 0)
  ...
  const totalPerf = impliedCapital > 0 ? totalUsd - impliedCapital : 0
  const totalPerfPct = impliedCapital > 0 ? (totalPerf / impliedCapital) * 100 : 0
  ...
  for (const p of positions) {
    const cat = CRYPTO_CATEGORY[p.ticker] || "Autres"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", borderBottom: "1px solid var(--rule)", marginBottom: 28 }}>
        ...
        <div style={{ padding: "16px 22px", borderRight: "1px solid var(--rule)" }}>
          <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "var(--ink3)", fontFamily: "var(--font-mono)", display: "flex", alignItems: "center" }}>
            Valeur portefeuille
            <InfoTip text="Somme positions × cours CoinGecko. Rafraîchi via Sync prix ou cron 22h UTC." />
          </div>
        ...
        <div style={{ padding: "16px 22px", borderRight: "1px solid var(--rule)" }}>
          <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "var(--ink3)", fontFamily: "var(--font-mono)", display: "flex", alignItems: "center" }}>
            Perf totale
            <InfoTip text="(Valeur actuelle − Capital investi) / Capital investi × 100." />
          </div>
          <div style={{ fontFamily: "var(--font-serif)", fontSize: 28, fontWeight: 700, color: impliedCapital > 0 ? (totalPerf >= 0 ? "var(--at-pos)" : "var(--at-neg)") : "var(--ink3)", marginTop: 6, letterSpacing: -0.5 }}>
            {impliedCapital > 0 ? `${totalPerf >= 0 ? "+" : ""}${fmtUsd(totalPerf)}` : "—"}
          </div>
          {impliedCapital > 0 && (
            <div style={{ fontFamily: "var(--font-serif)", fontSize: 11, fontStyle: "italic", color: "var(--ink3)", marginTop: 4 }}>
        ...
        <div style={{ padding: "16px 22px" }}>
          <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "var(--ink3)", fontFamily: "var(--font-mono)", display: "flex", alignItems: "center" }}>
            Cash / Stablecoins
            <InfoTip text="USDT, USDC, DAI, USDe — liquidités immédiatement déployables." />
          </div>
          <div style={{ fontFamily: "var(--font-serif)", fontSize: 28, fontWeight: 700, color: "var(--ink)", marginTop: 6, letterSpacing: -0.5 }}>
            {fmtUsd(stablecoinUsd)}
          </div>
        </div>
      </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "var(--font-mono)", fontSize: 12 }}>
                <thead>
                  <tr style={{ position: "sticky", top: 0, background: "var(--at-surface)", zIndex: 1 }}>
                    {["Ticker", "Nom", "Catégorie", "Poids", "Qté", "PRU", "Cours", "Valeur", "Var 24h", "P&L"].map((h, i) => (
                      <th key={h} style={{
                        padding: "10px 12px",
